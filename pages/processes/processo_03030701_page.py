        ...
        try:
            ...
            if not self._esperar_campo_habilitado_js("mapa", 5):
                return ExecutionResult(
                    ...
                    message="Formulário inicial não carregou.",
                )
            ...
            if not self._esperar_campo_habilitado_js("condNova", 15):
                ...
                return ExecutionResult(
                    status=ExecutionStatus.BUSINESS_FAILURE,
                    message="Nota bloqueada para edição ou não encontrada.",
                )
            ...
            if alerta_texto:
                return ExecutionResult(
                    status=ExecutionStatus.BUSINESS_FAILURE,
                    message=f"Condição inválida: {alerta_texto}",
                )

            if not self._esperar_campo_habilitado_js("BotSalvar", 5):
                return ExecutionResult(
                    status=ExecutionStatus.BUSINESS_FAILURE,
                    message="Botão Salvar bloqueado. Regra de negócio não permitiu a conversão.",
                )
            ...
            try:
                ...
                self.logger.debug(f"Não foi possível voltar ao conteúdo padrão após falha da nota {numero_nota}: {switch_error}")
            return ExecutionResult(
                status=ExecutionStatus.TECHNICAL_FAILURE,
                message=f"Falha sistêmica (Crash): {str(e)}",
            )
