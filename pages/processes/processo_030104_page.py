...
class Processo030104Page(RotinaPage):
    ...
    # --- METODO DE LEITURA (ESTATICO/OFFLINE) ---
        ...
        try:
            ...
            if not ok_mapa:
                return ExecutionResult(
                    ...
                    message=f"Mapa {mapa} inválido ou não carregado: {msg_mapa}",
                )
            ...
            for campo, val, trigger in campos_head:
                ...
                if not ok:
                    ...
                    return ExecutionResult(
                        status=ExecutionStatus.BUSINESS_FAILURE,
                        message=f"Erro no cabeçalho ({campo}): {msg}",
                    )
            ...
            self.logger.exception(f"Falha técnica ao digitar pedido do mapa {mapa}: {e}")
            try:
                self.executar_gatilho_e_aguardar("Cancelar();")
            except Exception as cancel_error:
                self.logger.debug(f"Falha ao cancelar pedido após erro técnico do mapa {mapa}: {cancel_error}")
            return ExecutionResult(
