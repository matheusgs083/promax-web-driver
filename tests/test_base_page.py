...
def test_lidar_com_alertas_drena_alertas_em_cascata(monkeypatch):
    ...
    monkeypatch.setattr("pages.base_page.WebDriverWait", FakeWebDriverWait)

    page = BasePage(driver)
    monkeypatch.setattr(page, "wait_for_no_alert", lambda timeout=0: True)

    page.lidar_com_alertas(tentativas=2, timeout=1, timeout_entre_alertas=1, max_alertas=5)
