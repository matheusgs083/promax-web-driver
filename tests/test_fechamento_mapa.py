...
def test_fechamento_mapa_executa_030330_quando_03030702_pede_comodato(monkeypatch):
    ...
    class Fake03030702Page:
        ...
        def carregar_mapa(self, _mapa, ponto_apoio=None):
            Fake03030702Page.chamadas += 1
            if Fake03030702Page.chamadas == 1:
                return ExecutionResult(
                    ExecutionStatus.BUSINESS_FAILURE,
                    "Alerta retornado ao carregar mapa: Comodato nao foi fechado atraves da rotina 03.03.30",
                    retry=False,
                    metadata={"alertas": ["Comodato nao foi fechado atraves da rotina 03.03.30"]},
                )
            return ExecutionResult(ExecutionStatus.SUCCESS, "03030702 carregada")
        ...
        def fechar_e_voltar(self):
            fechamentos_03030702.append(True)
            return FakeMenuPage()
    ...
    assert result.status == ExecutionStatus.SUCCESS
    assert rotinas_acessadas == ["030303", "030302", "03030702", "030330", "03030702"]
    assert fechamentos_03030702 == [True, True]
    assert cancelamentos_030330 == [True]
    assert fechamentos_030330 == [True]
    assert result.metadata["resultado_030330"]["status"] == "SUCESSO"
