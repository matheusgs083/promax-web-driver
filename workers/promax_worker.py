...
class WorkerConfig:
    ...
    def from_env(cls) -> WorkerConfig:
        default_driver_dir = PROJECT_ROOT
...
class PromaxWorker:
    ...
    def _run_claimed_job(self, job: Mapping[str, Any]) -> None:
        ...
        try:
            result = self.runner.run(
                ...
                on_event=lambda event: self._handle_partial_result_event(
                    job,
                    job_id,
                    lease_token,
                    event,
        ...
        *,
        ...
        try:
            ...
            if sync_scope == "030302" and not isinstance(response.get("conferencia"), Mapping):
                raise ValueError(
                    str(
                        response.get("conferencia_error")
                        or "A API nao confirmou a conferencia da 030302."
                    )
                )
    ...
    def _handle_partial_result_event(
        ...
        if str(event.get("event") or "").strip() != "promax_partial_result":
            return
        ...
        if self._sync_financeiro_fechamento_if_needed(
            job,
            job_id,
            lease_token,
            partial_result,
...
def load_project_env(project_root: Path = PROJECT_ROOT) -> Path | None:
    configured_path = (
        os.environ.get("PROMAX_ENV_FILE")
        or os.environ.get("BOT_ENV_FILE")
        or ".env"
    ).strip() or ".env"
...
def _resolve_lock_path(configured_path: str, *, filename: str) -> Path:
    ...
    for env_name in ("PROGRAMDATA", "LOCALAPPDATA"):
        ...
        if base:
            candidates.append(Path(base) / "promax-web-driver" / "locks" / filename)
    candidates.append(Path(tempfile.gettempdir()) / f"promax_web_driver_{filename}")
    for candidate in candidates:
        try:
            ...
            continue
    return Path(tempfile.gettempdir()) / f"promax_web_driver_{filename}"
