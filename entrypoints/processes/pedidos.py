...
from pages.processes.processo_030104_page import Processo030104Page
...
def iniciar_sessao():
    ...
    driver, menu_page = iniciar_sessao_padrao(logger, settings, settings.unidade_pedidos)
...
def main():
    ...
    pasta_logs = LOGS_DIR / "pedidos"
    pasta_logs.mkdir(parents=True, exist_ok=True)

    data_atual = dt.datetime.now().strftime("%Y%m%d")
    caminho_log_itens = str(pasta_logs / f"Detalhes_Itens_{data_atual}.csv")
    ...
    try:
        ...
        try:
            tracker.gerar_csv(str(pasta_logs))
