...
def _publicar_arquivo(origem: Path, destino_final: Path) -> None:
...
def mover_relatorios(origem, destino):
    ...
    if caminho_origem.is_file():
        _publicar_arquivo(caminho_origem, caminho_destino / caminho_origem.name)
        return
    ...
    if caminho_origem.is_dir():
        ...
        for item in caminho_origem.iterdir():
            if not item.is_file():
                continue
            _publicar_arquivo(item, caminho_destino / item.name)
